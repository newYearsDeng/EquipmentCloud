		...
		public synchronized void run() {
			...
			while (true) {
				try {
					...
					result += Udp_Help.bytesToHex(buffer, bytes);
					System.out.println("result*-***--**-*-"+result);

					handleBtBlueMessage(result);
					System.out.println("bytesToHex(buffer, buffer.length);"+Udp_Help.bytesToHex(buffer, bytes));
...
//                            -1, buffer).sendToTarget();
		...
		//FEFEFE6821000016200168940000BC16 设置成功回复
...
//			try{
//				while (true) {
//                    System.out.println(b.length+"*------********--"+mmInStream.available());
//					while(-1 != (dis.read(b))){
...
//
//					}
//					mHandler.obtainMessage(BluetoothChat.MESSAGE_READ, baos.toByteArray()).sendToTarget();
//					baos.reset();
//				}
//
//			}catch (Exception e){
